...
router.post('/register', [
  ...
  body('businessType').isIn(['single_location', 'franchise_owner'])
], async (req, res) => {
  try {
    ...
    const { email, password, firstName, lastName, businessName, phone, plan, businessType, franchiseSize } = req.body;
    ...
    const role = businessType === 'franchise_owner' ? 'franchise_owner' : 'owner';
    const isMainOwner = businessType === 'franchise_owner';
    ...
    const user = new User({
      ...
      franchiseSize: businessType === 'franchise_owner' ? franchiseSize : null,
      isMainOwner: isMainOwner
    });

    await user.save();

    // Note: PWA authentication is handled via proxy endpoint /api/pwa-auth/login
    console.log(`✅ User registered: ${user.email} (${user.role}) - PWA can authenticate via Marketing Website proxy`);
    ...
    const token = jwt.sign(
      { 
        ...
        ownerId: user.ownerId || (user.role === 'owner' || user.role === 'franchise_owner' ? user._id : undefined),
