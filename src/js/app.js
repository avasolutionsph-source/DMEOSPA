...
function closeModal(modalId) {
    if (window.app) {
        ...
        // Fallback for direct modal closing
        const modal = document.getElementById(modalId);
        if (modal) {
            modal.style.display = 'none';
...
// Global modal functions for specific components
function showInventoryModal() {
    ...
    if (form) form.reset();
    if (title) title.textContent = 'Add Spa Supply';
    
    // Clear any editing state
    if (window.inventoryManager) {
        window.inventoryManager.editingItem = null;
    }
}
...
function showEmployeeModal() {
    ...
    if (window.employeesManager) {
        window.employeesManager.editingEmployee = null;
    }
}
...
function showRoomModal() {
    ...
    if (form) form.reset();
    if (title) title.textContent = 'Add Room';
    
    // Clear any editing state
    if (window.roomsManager) {
        window.roomsManager.editingRoom = null;
    }
}

// Dropdown handler for custom options
function handleDropdownChange(selectElement, customInputId) {
...
function showGiftCertificateModal() {
    console.log('🎁 Opening gift certificate modal...');
    openModal('giftCertificateModal');
}

function generateGCReport() {
    console.log('📊 Generating gift certificate report...');
    if (window.giftCertificatesManager && window.giftCertificatesManager.generateReport) {
        window.giftCertificatesManager.generateReport();
    } else {
        showNotification('Gift certificate reporting is not available', 'warning');
    }
}

function bulkGCOperations() {
    console.log('📋 Opening bulk gift certificate operations...');
    if (window.giftCertificatesManager && window.giftCertificatesManager.showBulkOperations) {
        window.giftCertificatesManager.showBulkOperations();
    } else {
        showNotification('Bulk operations are not available', 'warning');
    }
}

// POS Functions
function holdTransaction() {
    console.log('⏸️ Holding current transaction...');
    if (window.posSystem && window.posSystem.holdTransaction) {
        window.posSystem.holdTransaction();
    } else {
        showNotification('POS system is not available', 'warning');
    }
}

function proceedToCheckout() {
    console.log('🛒 Proceeding to checkout...');
    if (window.posSystem && window.posSystem.proceedToCheckout) {
        window.posSystem.proceedToCheckout();
    } else if (window.pos && window.pos.proceedToCheckout) {
        window.pos.proceedToCheckout();
    } else {
        showNotification('Checkout system is not available', 'warning');
    }
}
...
function clearChatHistory() {
    ...
    if (window.chatbot && window.chatbot.clearHistory) {
        window.chatbot.clearHistory();
    } else {
        showNotification('Chat history cleared', 'success');
        const chatMessages = document.getElementById('chatMessages');
        if (chatMessages) {
            chatMessages.innerHTML = '<div class="chat-message bot"><div class="message-content"><p>Chat history cleared. How can I help you?</p></div></div>';
        }
    }
}
...
function askQuickQuestion(question) {
    ...
    if (window.chatbot && window.chatbot.askQuestion) {
...
function saveAllSettings() {
    ...
    if (window.settingsManager && window.settingsManager.saveAllSettings) {
...
function exportData() {
    ...
    if (window.app && window.app.exportData) {
        ...
        showNotification('Data export feature is not available', 'warning');
    }
}
...
function importData() {
    ...
    if (window.app && window.app.importData) {
        ...
        showNotification('Data import feature is not available', 'warning');
    }
}
...
function refreshSubscription() {
    ...
    if (window.entitlementsSystem && window.entitlementsSystem.checkSubscriptionStatus) {
        window.entitlementsSystem.checkSubscriptionStatus();
        showNotification('Subscription status refreshed', 'success');
    } else {
        showNotification('Subscription system is not available', 'warning');
    }
}

// POS System Functions (for checkout modal)
window.posSystem = {
    validateGiftCertificate: function() {
        console.log('🎁 Validating gift certificate...');
        const gcNumber = document.getElementById('gcControlNumber').value;
        const gcStatus = document.getElementById('gcStatus');
        
        if (!gcNumber.trim()) {
            gcStatus.innerHTML = '<span style="color: red;">Please enter a control number</span>';
            return;
        }
        
        // Simulate validation (replace with actual validation logic)
        gcStatus.innerHTML = '<span style="color: orange;">Validating...</span>';
        setTimeout(() => {
            gcStatus.innerHTML = '<span style="color: green;">✅ Valid certificate - ₱500.00 available</span>';
        }, 1000);
    },
    
    toggleSeniorPWDFields: function() {
        console.log('👴 Toggling senior/PWD discount fields...');
        const checkbox = document.getElementById('applySeniorPWD');
        const fields = document.getElementById('seniorPWDFields');
    ...
    applyPromoDiscount: function() {
        console.log('🎫 Applying promo discount...');
        const promoSelect = document.getElementById('promoDiscount');
        const customField = document.getElementById('customDiscountField');
        
        if (customField) {
            customField.style.display = promoSelect.value === 'custom' ? 'block' : 'none';
        }
    }
};
