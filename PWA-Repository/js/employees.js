...
// ========================================
// DEBUG FUNCTIONS FOR EMPLOYEE SYNC
// ========================================

// DEBUG: Manual employee sync trigger for testing
window.debugSyncEmployees = async function() {
    console.log('🔧 [DEBUG] Manual employee sync triggered');
    
    if (!window.syncManager) {
        console.error('❌ [DEBUG] Sync manager not available');
        return;
    }
    
    if (typeof window.syncManager.syncEmployees !== 'function') {
        console.error('❌ [DEBUG] syncEmployees function not available');
        return;
    }
    
    try {
        console.log('🚀 [DEBUG] Starting employee sync...');
        await window.syncManager.syncEmployees();
        console.log('✅ [DEBUG] Employee sync completed');
    } catch (error) {
        console.error('❌ [DEBUG] Employee sync failed:', error);
    }
};

// DEBUG: Check current employee data in IndexedDB
window.debugEmployeeData = async function() {
    console.log('🔍 [DEBUG] Checking employee data in IndexedDB');
    ...
    try {
        ...
        console.log('📊 [DEBUG] Employees in IndexedDB:', employees.length);
        ...
        employees.forEach((emp, index) => {
            console.log(`${index + 1}. ${emp.name}:`, {
                ...
                totalSales: emp.totalSales || 0,
                totalCommission: emp.totalCommission || 0,
                totalTransactions: emp.totalTransactions || emp.transactionCount || 0,
                syncStatus: emp.syncStatus || 'unknown',
                lastSyncDate: emp.lastSyncDate || 'never'
            });
        });
        
        return employees;
    } catch (error) {
        console.error('❌ [DEBUG] Error checking employee data:', error);
    }
};

// DEBUG: Force sync all data including employees
window.debugSyncAll = async function() {
    console.log('🔧 [DEBUG] Manual full sync triggered');
    ...
    try {
        console.log('🚀 [DEBUG] Starting full sync (including employees)...');
        await window.syncManager.syncAll();
        console.log('✅ [DEBUG] Full sync completed');
    } catch (error) {
        console.error('❌ [DEBUG] Full sync failed:', error);
    }
};

// DEBUG: Check sync manager status
window.debugSyncStatus = function() {
    console.log('🔧 [DEBUG] Sync Manager Status Check:');
    console.log('   - window.syncManager exists:', !!window.syncManager);
    console.log('   - window.dataSync exists:', !!window.dataSync);
    
    if (window.syncManager) {
        console.log('   - syncManager.syncAll exists:', typeof window.syncManager.syncAll);
        console.log('   - syncManager.syncEmployees exists:', typeof window.syncManager.syncEmployees);
        console.log('   - syncManager.isOnline:', window.syncManager.isOnline);
        console.log('   - syncManager.syncInProgress:', window.syncManager.syncInProgress);
    }
    
    if (window.dataSync) {
        console.log('   - dataSync.syncAll exists:', typeof window.dataSync.syncAll);
    }
};

// DEBUG: Force trigger sync with detailed logging
window.debugForceSync = async function() {
    console.log('🚀 [DEBUG] FORCE SYNC - Starting...');
    
    // Check if sync manager exists
    if (!window.syncManager) {
        console.error('❌ [DEBUG] window.syncManager is not defined!');
        return;
    }
    
    // Check if sync function exists
    if (typeof window.syncManager.syncEmployees !== 'function') {
        console.error('❌ [DEBUG] syncEmployees function does not exist');
        return;
    }
    
    try {
        console.log('📊 [DEBUG] Calling syncEmployees directly...');
        await window.syncManager.syncEmployees();
        console.log('✅ [DEBUG] Direct syncEmployees call completed');
    } catch (error) {
        console.error('❌ [DEBUG] Direct syncEmployees call failed:', error);
    }
    
    try {
        console.log('🔄 [DEBUG] Calling syncAll...');
        await window.syncManager.syncAll();
        console.log('✅ [DEBUG] syncAll completed');
    } catch (error) {
        console.error('❌ [DEBUG] syncAll failed:', error);
    }
};

console.log('🛠️ [DEBUG] Employee debug functions loaded:');
console.log('   - window.debugEmployeeData() - Check employee data in IndexedDB');
console.log('   - window.debugSyncEmployees() - Manual employee sync');
console.log('   - window.debugSyncAll() - Full sync including employees');
console.log('   - window.debugSyncStatus() - Check sync manager status');
console.log('   - window.debugForceSync() - Force sync with detailed logging');
console.log('✅ [EMPLOYEES] EmployeeManager created and made globally accessible');

// Load employees when page is shown
window.loadEmployees = async function() {
    console.log('🚀 [LOAD-EMPLOYEES] Starting employee page load...');
    
    // AUTO-DEBUG: Test employee API immediately
    if (window.HybridAPIClient && window.HybridAPIClient.quickTestEmployees) {
        console.log('🧪 [AUTO-DEBUG] Running automatic employee test...');
        try {
            const testResult = await window.HybridAPIClient.quickTestEmployees();
            console.log('🧪 [AUTO-DEBUG] Test completed:', testResult);
        } catch (error) {
            console.log('❌ [AUTO-DEBUG] Test failed:', error);
        }
    }
