...
class GiftCertificateManager {
    ...
    async loadCertificates() {
        try {
            ...
            if (window.db) {
                const dbCertificates = await window.db.getAll('giftCertificates');
                
                // Migration: Check for certificates in old location (products table)
                const allProducts = await window.db.getAll('products');
                const oldCertificates = allProducts.filter(item => item.type === 'gift_certificate');
                
                // Migrate old certificates to correct store
                for (const oldCert of oldCertificates) {
                    try {
                        // Add to correct store
                        await window.db.add('giftCertificates', oldCert);
                        // Remove from old store
                        await window.db.delete('products', oldCert.id);
                        console.log(`Migrated certificate ${oldCert.controlNumber} to correct store`);
                    } catch (error) {
                        console.warn(`Could not migrate certificate ${oldCert.controlNumber}:`, error);
                    }
                }
                
                // Reload certificates after migration
                const finalCertificates = await window.db.getAll('giftCertificates');
                ...
                [...finalCertificates, ...localCertificates].forEach(cert => {
                ...
                <div class="certificate-details" style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <div class="certificate-actions" style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                        <button class="btn-view" data-action="view" data-cert-id="${cert.id}" style="
                            ...
                            background: linear-gradient(135deg, #800020 0%, #600015 100%) !important; 
        ...
        if (result.valid) {
            ...
            resultDiv.innerHTML = `
                <div class="alert alert-success" style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); color: #059669; padding: 1.5rem; border-radius: 12px; margin: 1rem 0;">
                    <h4 style="color: #800020; margin-bottom: 1rem; font-size: 1.2rem;"><i class="fas fa-check-circle" style="color: #800020; margin-right: 0.5rem;"></i>Certificate is Valid</h4>
            ...
            resultDiv.innerHTML = `
                <div class="alert alert-error" style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: #dc2626; padding: 1.5rem; border-radius: 12px; margin: 1rem 0;">
                    <h4 style="color: #800020; margin-bottom: 0.75rem; font-size: 1.2rem;"><i class="fas fa-times-circle" style="color: #800020; margin-right: 0.5rem;"></i>Certificate Invalid</h4>
