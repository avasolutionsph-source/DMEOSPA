...
class SettingsManager {
    ...
    setupEventListeners() {
        ...
        // Download app button (reuse existing install functionality)
        const downloadAppBtn = document.getElementById('downloadAppBtn');
        if (downloadAppBtn && !downloadAppBtn.hasAttribute('data-listener')) {
            downloadAppBtn.addEventListener('click', async () => {
                const updateStatus = document.getElementById('updateStatus');
                
                if (updateStatus) {
                    updateStatus.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Preparing download...';
                }
                
                if (window.app && window.app.installApp) {
                    // Use the improved installApp function
                    await window.app.installApp();
                } else {
                ...
                if (updateStatus) {
                    setTimeout(() => {
                        updateStatus.innerHTML = '<i class="fas fa-info-circle"></i> Ready to download or install updates';
                    }, 3000);
                }
            });
            downloadAppBtn.setAttribute('data-listener', 'true');
        }

        // Check updates button (reuse existing update check functionality)
        const checkUpdatesBtn = document.getElementById('checkUpdatesBtn');
        if (checkUpdatesBtn && !checkUpdatesBtn.hasAttribute('data-listener')) {
            checkUpdatesBtn.addEventListener('click', async () => {
                ...
                if (window.checkForUpdates) {
                    // Use existing auto-updater functionality
                    window.checkForUpdates();
                    setTimeout(() => {
                        if (updateStatus) {
                            updateStatus.innerHTML = '<i class="fas fa-check-circle"></i> Update check completed';
                        }
                        showNotification('Update check completed', 'success');
                    }, 2000);
                } else {
                    if (updateStatus) {
                        updateStatus.innerHTML = '<i class="fas fa-exclamation-circle"></i> Update system not available';
                    }
                    showNotification('Update system not available', 'warning');
                }
            });
            checkUpdatesBtn.setAttribute('data-listener', 'true');
        }
        ...
        if (syncDataBtn && !syncDataBtn.hasAttribute('data-listener')) {
            syncDataBtn.addEventListener('click', async () => {
                await this.syncDataFromServer();
            });
            syncDataBtn.setAttribute('data-listener', 'true');
        }
    ...
    async addPerformanceSection() {
        ...
        // Target the settings page specifically, not payroll settings
        const settingsPage = document.getElementById('settings');
        if (!settingsPage) return;
        
        const settingsSection = settingsPage.querySelector('.settings-section');
        ...
        try {
            ...
            if (businessName) {
                ...
                if (businessNameInput) {
                    businessNameInput.value = businessName.value;
                }
            }
    ...
    async syncDataFromServer() {
        try {
            ...
            if (window.runFullDataSync) {
                await window.runFullDataSync();
                
                // Update last sync info
                if (lastSyncInfo) {
                    const now = new Date().toLocaleString();
                    lastSyncInfo.innerHTML = `<i class="fas fa-check-circle" style="color: green;"></i> Last sync: ${now}`;
                }
                
                // Show success notification
                showNotification('Data synced successfully! All information is now available offline.', 'success');
                ...
                throw new Error('Full data sync function not available');
            }
            ...
            const lastSyncInfo = document.getElementById('lastSyncInfo');
            if (lastSyncInfo) {
                lastSyncInfo.innerHTML = `<i class="fas fa-exclamation-triangle" style="color: red;"></i> Sync failed - using offline data`;
            }
            
            showNotification('Data sync failed. Using offline data. Check your connection.', 'error');
        } finally {
            // Re-enable button
            const syncBtn = document.getElementById('syncDataBtn');
            if (syncBtn) {
                syncBtn.disabled = false;
                syncBtn.innerHTML = '<i class="fas fa-sync"></i> Sync Data from Server';
            }
        }
    }
    ...
    async updateLastSyncDisplay() {
        try {
            ...
            if (lastSync && lastSync.value) {
                ...
                lastSyncInfo.innerHTML = `<i class="fas fa-clock"></i> Last sync: ${syncDate.toLocaleString()}`;
            } else {
                lastSyncInfo.innerHTML = `<i class="fas fa-clock"></i> Last sync: Never`;
            }
        } catch (error) {
            console.error('Failed to update last sync display:', error);
        }
    }
