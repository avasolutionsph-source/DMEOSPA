...
class CustomerManager {
    ...
    async loadCustomers() {
        ...
        try {
            ...
            console.log('👥 [CUSTOMER-MANAGER] HybridAPIClient available:', !!window.HybridAPIClient);
            console.log('👥 [CUSTOMER-MANAGER] Online status:', navigator.onLine);
            ...
            if (!window.HybridAPIClient) {
                ...
                // Fallback to IndexedDB if HybridAPIClient not available
                ...
                console.log('👥 [CUSTOMER-MANAGER] HybridAPIClient response:', {
    ...
    async saveCustomer() {
        ...
        try {
            ...
            if (!window.HybridAPIClient) {
                ...
                console.log('💾 [CUSTOMER-MANAGER] Save result:', {
                    success: result.success,
                    source: result.source,
                    hasData: !!result.data
                });
            ...
            console.error('❌ [CUSTOMER-MANAGER] Error saving customer:', error);
    ...
    async editCustomer(customerId) {
        console.log('✏️ [CUSTOMER-MANAGER] Editing customer:', customerId);

        try {
            // Find customer (check both id and _id fields)
            ...
            console.error('❌ [CUSTOMER-MANAGER] Error editing customer:', error);
            showError('Failed to open customer for editing');
        }
    }

    showEditCustomerModal(customer) {
        // Create and show modal for editing customer
        const modalHtml = `
            <div id="editCustomerModal" class="modal active">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2><i class="fas fa-user-edit"></i> Edit Customer</h2>
                        <button class="modal-close" onclick="window.customerManager.closeEditCustomerModal()">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form id="editCustomerForm">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label>First Name *</label>
                                    <input type="text" id="editCustomerFirstName" class="form-input" value="${customer.firstName || ''}" required>
                                </div>
                                <div class="form-group">
                                    <label>Last Name *</label>
                                    <input type="text" id="editCustomerLastName" class="form-input" value="${customer.lastName || ''}" required>
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Phone Number *</label>
                                <input type="tel" id="editCustomerPhone" class="form-input" placeholder="09XXXXXXXXX" value="${customer.phone || ''}" required>
                            </div>
                            <div class="form-group">
                                <label>Email (optional)</label>
                                <input type="email" id="editCustomerEmail" class="form-input" placeholder="customer@email.com" value="${customer.email || ''}">
                            </div>
                            <div class="form-group">
                                <label>Address (optional)</label>
                                <textarea id="editCustomerAddress" class="form-input" rows="3" placeholder="Customer address">${customer.address || ''}</textarea>
                            </div>
                            <div class="form-group">
                                <label>Notes (optional)</label>
                                <textarea id="editCustomerNotes" class="form-input" rows="2" placeholder="Special notes about the customer">${customer.notes || ''}</textarea>
                            </div>
                        </form>
                    </div>
                    <div class="modal-actions">
                        <button type="button" class="btn btn-secondary" onclick="window.customerManager.closeEditCustomerModal()">Cancel</button>
                        <button type="button" class="btn btn-primary" onclick="window.customerManager.updateCustomer()">
    ...
    async updateCustomer() {
        ...
        if (this.isDuplicatePhone(phone, customerId)) {
            const existingCustomer = this.customers.find(c => c.phone === phone && c.id !== customerId);
            showError(`Phone number ${phone} is already used by ${existingCustomer.firstName} ${existingCustomer.lastName}`);
            return;
        }

        try {
            console.log('💾 [CUSTOMER-MANAGER] Updating customer...');
            console.log('💾 [CUSTOMER-MANAGER] HybridAPIClient available:', !!window.HybridAPIClient);
            console.log('💾 [CUSTOMER-MANAGER] Online status:', navigator.onLine);
            ...
            if (!window.HybridAPIClient) {
                ...
                if (customer) {
                    ...
                    // Trigger sync if possible
                    if (window.syncManager?.isOnline) {
                        console.log('🔄 Triggering customer sync after update');
                        window.syncManager.triggerSync();
                    }
                } else {
                ...
                console.log('💾 [CUSTOMER-MANAGER] Update result:', {
                    success: result.success,
                    source: result.source,
                    hasData: !!result.data
                });
                ...
                if (result.success) {
                    ...
                    console.log(`✅ [CUSTOMER-MANAGER] Customer updated via ${result.source}`);
                } else {
            ...
            console.error('❌ [CUSTOMER-MANAGER] Error updating customer:', error);
    ...
    async deleteCustomer(customerId) {
        ...
        try {
            console.log('🗑️ [CUSTOMER-MANAGER] Deleting customer:', customerId);
            console.log('🗑️ [CUSTOMER-MANAGER] HybridAPIClient available:', !!window.HybridAPIClient);
            console.log('🗑️ [CUSTOMER-MANAGER] Online status:', navigator.onLine);
            ...
            if (!window.HybridAPIClient) {
                ...
                console.log('🗑️ [CUSTOMER-MANAGER] Delete result:', {
                    success: result.success,
                    source: result.source
                });
                ...
                console.log(`✅ [CUSTOMER-MANAGER] Customer deleted via ${result.source}`);
            }
            ...
            console.error('❌ [CUSTOMER-MANAGER] Error deleting customer:', error);
